...
contract SafeMath {
    ...
    uint constant preIcoPrice = 4000;
    uint constant icoPrice = 2400;
...
contract CabCoinToken is StandardToken, SafeMath {
    /*
     * Token meta data
     */
...
contract CabCoinIco is SafeMath {
    /*
     * ICO meta data
     */
    CabCoinToken public cabcoinToken;
    ...
    function CabCoinIco(address _icoManager, address _preIcoToken) {
        assert(_preIcoToken != 0x0);
        assert(_icoManager != 0x0);

        cabcoinToken = new cabcoinToken(this);
    ...
    function importTokens(address _account) {
        ...
        if (preIcoBalance > 0) {
            cabcoinToken.emitTokens(_account, preIcoBalance);
    ...
    function buyTokens(address _buyer) private {
        ...
        cabcoinToken.emitTokens(_buyer, tokensToEmit);
    ...
    function burnTokens(address _from, uint _value) onlyManager notMigrated {
        cabcoinToken.burnTokens(_from, _value);
    }
    ...
    function sendTokensToBountyOwner() onlyManager whenInitialized {
        ...
        cabcoinToken.emitTokens(bountyOwner, bountyTokens);

        sentTokensToBountyOwner = true;
    }

    /// @dev Send tokens to founders. Can be sent only after cabcoinToken.rewardTime() (2018.02.04 0:00 UTC)
    function sendTokensToFounders() onlyManager whenInitialized {
        ...
        cabcoinToken.emitTokens(founder1, founderReward);
        cabcoinToken.emitTokens(founder2, founderReward);
        cabcoinToken.emitTokens(founder3, founderReward);
        cabcoinToken.emitTokens(founder4, founderReward);
